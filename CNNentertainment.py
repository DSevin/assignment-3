...
class ScrapeNews(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        all_news = response.css("div.container_lead-plus-headlines-with-images__item.container_lead-plus-headlines-with-images__item--type-section a")
        for news in all_news:
            title = news.css('div.container__headline span ::text').get()
    ...
    def extract_content(self, response):
        ...
        content = ' '.join(response.css('div.article__content p.paragraph.inline-placeholder.vossi-paragraph-primary-core-light ::text').extract()).strip()
        return content